...
def main() -> int:
    required = [
        ...
        ROOT / "system" / "runtime" / "repository_state" / "latest.json",
        ROOT / "system" / "runtime" / "repository_state" / "latest.md",
        ROOT / "system" / "orchestrator" / "repository_state_builder.py",
        ROOT / "system" / "tests" / "test_repository_state_builder.py",
    ]
    missing = [str(path.relative_to(ROOT)) for path in required if not path.exists()]
    if missing:
        print("FAIL: missing repository state projection files:", ", ".join(missing))
        return 1
    payload = json.loads((ROOT / "system" / "runtime" / "repository_state" / "latest.json").read_text(encoding="utf-8"))
