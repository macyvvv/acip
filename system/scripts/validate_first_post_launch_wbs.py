...
def main() -> int:
    required = [
        ...
        ROOT / "runtime" / "planning" / "first_post_launch_wbs.json",
        ROOT / "runtime" / "planning" / "background_system_image.json",
        ROOT / "tests" / "test_first_post_launch_wbs.py",
    ]
    missing = [str(path.relative_to(ROOT)) for path in required if not path.exists()]
    if missing:
        print("FAIL: missing first post launch WBS files:", ", ".join(missing))
        return 1
    wbs = json.loads((ROOT / "runtime" / "planning" / "first_post_launch_wbs.json").read_text(encoding="utf-8"))
    bg = json.loads((ROOT / "runtime" / "planning" / "background_system_image.json").read_text(encoding="utf-8"))
