...
ADR-0033 (Level 1) gave the business agent platform automatic next-task proposal, but every business_role_task scope — from any business, any role — still shared a single, repo-wide handoff/approval slot (`platform/system/runtime/agent_handoff/latest.json` / `approval.json`), the same one the pre-existing issue/draft repo-dev flow uses. Only one scope could be "the" currently-approvable candidate at a time. Level 1's `business_agent_trigger.py` worked around this with an anti-clobber guard (skip activation if a different, currently-approved-and-pending scope existed), and `ApprovalConsoleService`/`propose_task.py` inherited the same single-slot assumption — `propose_task.py` had no guard at all, silently overwriting whatever was pending. A shared `platform/system/runtime/agent_execution/latest.json` had the same problem for execution results.

## Proposed Design (adopted)

Each `(business_id, role_id, task_id)` scope gets its own handoff/approval/execution-result files, mirroring the existing per-task artifact directory convention (`platform/system/runtime/business_agents/{business_id}/{role_id}/{task_id}/`):

- `platform/system/runtime/agent_handoff/scopes/{business_id}/{role_id}/{task_id}/{handoff,approval}.json` — `platform/system/core/business_agent_handoff.py::scope_dir()`/`write_business_agent_handoff()`/`load_business_agent_handoff()`.
- `platform/system/runtime/agent_execution/scopes/{business_id}/{role_id}/{task_id}/latest.json` — `ApprovedAutonomousExecution._write_runtime()` branches on whether the handoff carries `business_id`/`role_id`.
- New `platform/system/core/agent_execution_approval.py::evaluate_business_agent_scope_approval(business_id, role_id, task_id, base_path)`, sharing its field-matching validation with the unchanged `evaluate_execution_approval()` via an extracted private helper.
...
- New `platform/system/core/file_lock.py` (a `fcntl.flock`-based context manager, short timeout, hard error rather than silent retry), applied only to the two files that remain genuinely shared across all businesses: `business_agent_task_queue.py`'s `queue.json` and `kpi_store.py`'s `kpi.json`. Per-task files need no locking — no two processes legitimately touch the same task's file at once.
...
- New: `platform/system/core/file_lock.py`, this ADR, and the per-task/per-scope directory trees under `platform/system/runtime/agent_handoff/scopes/` and `platform/system/runtime/agent_execution/scopes/`.
- Modified (additive/parameterized, not rewritten): `platform/system/core/business_agent_handoff.py`, `platform/system/core/agent_execution_approval.py`, `platform/system/core/business_agent_task_queue.py`, `platform/system/core/kpi_store.py`, `platform/system/core/business_agent_trigger.py` (simplified — guard removed), `platform/system/core/agent_role_registry.py` (`pdca.next_roles`), `platform/system/core/approved_autonomous_execution.py`, `platform/system/platform/scripts/agent/set_execution_approval.py`, `platform/system/platform/scripts/agent/run_approved_autonomous_execution.py`, `platform/app/tools/approval_console_mvp/service.py`.
