...
- `platform/system/platform/scripts/publishing/providers.py` — provider/registry pattern mirroring `platform/system/platform/scripts/analytics/providers.py`; only `DryRunProvider` implemented, real per-platform adapters (`providers_x.py`/`providers_threads.py`/`providers_notecom.py`) deliberately unbuilt and unregistered until credentials exist.
- `platform/system/core/publishing_policy.py` + hand-authored, PR-reviewed `platform/system/runtime/publishing/policy.json` — standing, revocable authorization per `(business_id, platform)`: enabled flag, allowed source roles (`marketing`/`doc_creation` only), daily/weekly post caps, an affiliate-disclosure-tag requirement. Absence of an entry always means "not authorized" (fail closed).
- `platform/system/platform/scripts/publishing/finalize_content.py` — a human, after a task has executed successfully, distills its raw (often multi-option) output into the one exact string that may post, writing `platform/system/runtime/publishing/finalized/{business_id}/{role_id}/{task_id}/{platform}.json` with a hash of the source execution content. This artifact — not execution approval, and not a proposal-time flag — is the sole eligibility signal a scheduled run acts on.
- `platform/system/core/publishing_control.py` — a second, dedicated kill switch (`is_publishing_paused`/`pause_publishing`/`resume_publishing`), separate from the existing task-proposal switch (`business_agent_automation_control.py`) but checked *in addition to* it: the scheduler stops on either being engaged. No manual-override/force path exists for either.
- `platform/system/core/publishing_state.py` — dedup + daily/weekly counters, sharded per `(business_id, platform)` (not one shared file), locked via the existing `platform/system/core/file_lock.py`, fresh-read-inside-the-lock to prevent double-posting across concurrent runs. A corrupted state file hard-fails the run for that scope rather than being read as empty.
- `platform/system/platform/scripts/publishing/run_scheduled_publish.py` — finds all finalized-content candidates, evaluates policy/cap/dedup/kill-switch/content-integrity/disclosure eligibility, publishes eligible ones via the resolved provider, and writes a JSON+MD audit trail (`platform/system/runtime/publishing/audit/`), surfaced in the existing Approval Console via a new banner.
...
- New: `platform/system/platform/scripts/publishing/` (`providers.py`, `finalize_content.py`, `run_scheduled_publish.py`, `pause_publishing.py`, `resume_publishing.py`), `platform/system/core/publishing_policy.py`, `platform/system/core/publishing_control.py`, `platform/system/core/publishing_state.py`, this ADR, and the `platform/system/runtime/publishing/` tree.
- Modified (additive only): `platform/app/tools/approval_console_mvp/service.py` (new banner), `platform/docs/current/BUSINESS_AGENT_AUTOMATION_READINESS.md` (Level 3c section + Forbidden Now updates), `platform/contracts/roles/MARKETING_OUTPUT_CONTRACT.md`, `platform/contracts/roles/DOC_CREATION_OUTPUT_CONTRACT.md`.
- Unaffected: `platform/system/orchestrator/business_agent_execution_adapter.py`, `platform/system/core/agent_execution_approval.py`, `platform/system/core/business_agent_automation_control.py` (read from, not modified), `platform/system/core/business_agent_task_queue.py`, `platform/system/core/business_agent_handoff.py` (no schema change — the `publish_candidate` flag idea was rejected, so these need zero changes), all `pluggable_provider` roles, Levels 0-2 in full.

## Migration Cost

None. Purely additive/greenfield — `platform/system/runtime/publishing/` is a brand-new tree, and `policy.json` ships absent by default (no policy = fail closed everywhere). The operator adds the first pilot policy entry as their own separate, reviewed commit.
