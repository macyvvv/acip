...
per `platform/adr/ADR-0038` but not yet wired to cron/launchd). Governance Layer
Overhaul (`platform/adr/ADR-0037`) complete as of its 2026-07-14 addendum.
...
`platform/system/agent_runtime/role_prompts/*.md` (automated-execution authority)
and `.claude/agents/*.md` (interactive-session authority, `platform/adr/ADR-0039`)
...
tasks in `platform/system/runtime/business_agent_tasks/queue.json` (mostly
...
see `platform/adr/ADR-0037` for why this was added as an explicit convention.
