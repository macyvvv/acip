...
`platform/system/runtime/workers/` and `platform/system/runtime/governor/` were flagged
...
Confirmed via `git grep` across `*.py`, `*.yml`, `platform/docs/`, `platform/system/`, and
`platform/app/` before archiving: zero code or workflow references to either
directory's path.
...
| `governor/` | `GOVERNOR_RECOMMENDATIONS.md`, `governor_recommendations.json` | The JSON stub's `active_ep`/`next_ep` fields (`EP-0108`/`EP-0109`) were stale relative to current `platform/docs/current/STATE.md`, with an empty `candidates: []` that was never populated. `platform/docs/current/GOVERNOR_RECOMMENDATION_SSOT.md` described this path as a "source of truth" that no code ever read from or wrote to. |

`platform/docs/current/GOVERNOR_RECOMMENDATION_SSOT.md` is updated in the same
