...
def test_repository_os_v1_completion_review_contains_core_sections() -> None:
    path = Path("platform/docs/current/REPOSITORY_OS_V1_COMPLETION_REVIEW.md")
...
def test_repository_os_v1_review_json_is_machine_readable() -> None:
    payload = json.loads(Path("platform/system/runtime/reviews/repository_os_v1_completion_review.json").read_text(encoding="utf-8"))
    assert payload["repository_os_v1"] is True
    assert "platform/system/orchestrator/execution_kernel.py" in payload["entry_points"]
