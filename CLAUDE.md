...
- `main` への直接pushは禁止。常に feature branch → commit → push → PR →
  ...
  `bash platform/system/platform/scripts/git/install_hooks.sh` で有効化（`.git/hooks/`は
...
- リポジトリ全体の自己検証: `python platform/system/platform/scripts/validate_all.py`
- CIの `validate-all.yml` 相当の統合ステータス: `bash platform/system/platform/scripts/check_repo_os_status.sh`
