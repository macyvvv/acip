...
- The KPI store itself (`system/core/kpi_store.py`, `system/runtime/knowledge/kpi.json`) already exists and is wired into every business-agent role execution via `update_business_agent_kpi()` — run/success/failure counts are real for several roles today. Only the `metrics` sub-field has been empty, because `analytics` had never been invoked with a non-`dry_run` provider before `git_activity` existed. A 2026-07-14 repo-process consultation initially reported "no KPI store exists" — that was inaccurate; the store exists, only real metrics were missing. Corrected here so the record doesn't repeat that error.
