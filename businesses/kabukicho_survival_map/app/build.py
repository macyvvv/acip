...
FAQ_ITEMS = [
    ...
    (
        "歌舞伎町で24時間使えるトイレはどこですか？",
        "「西武新宿駅前公衆便所」「Haijia(東京都健康プラザ ハイジア)1階トイレ」「四季の路(新宿遊歩道公園)トイレ」など、「24h」タグ付きの数か所が24時間利用できます。それ以外の施設内トイレは営業時間内のみで、東急歌舞伎町タワーのトイレは認証式でテナント利用者限定です。カテゴリ「トイレ」からタグで絞り込んで確認してください。",
    ),
    (
        "歌舞伎町で終電を逃したらどうすればいいですか？",
        "「朝まで過ごす」モードに切り替えると、宿泊・コンビニ・トイレを中心に、営業中の一部のカラオケ・シーシャバーも含めた候補を状況別に表示します。個別の営業時間・24時間対応かどうかは各POIカードのタグと最終確認日で確認してください(カラオケ・シーシャバーは店舗によって深夜営業時間が異なります)。",
    ),
]
