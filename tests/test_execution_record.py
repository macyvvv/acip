from orchestrator.execution_record import build_worker_execution_record
...
def test_build_worker_execution_record() -> None:
    task = Task(
        id="EP-0106:1",
        artifact="orchestrator/queue_transition.py",
        owner="Codex",
        instruction="implement",
        done_conditions="tests pass",
    )
    result = Result(
        artifacts=["orchestrator/queue_transition.py"],
        files_changed=["orchestrator/queue_transition.py"],
        review_notes=["ok"],
    )
    ...
    assert record.files_changed == ["orchestrator/queue_transition.py"]
