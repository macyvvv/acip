...
## Agents you manage
- `scenario-writing` (`.claude/agents/scenario-writing.md`) — the pipeline's input stage; verify its output matches the content spec (`platform/system/platform/scripts/platform/somia/content_spec.py`) before handing it downstream.
...
## What you own
- `platform/system/platform/scripts/platform/somia/fal_client.py`, `providers.py`, `providers_illustrious_kling.py`, `providers_pika.py`, `providers_kling.py`, `render_content.py` — the actual API-calling plumbing (auth via env var, `fal_client.submit`/`await_result`/`upload`/`download`, retry/error handling).
