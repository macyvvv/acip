...
- `businesses/somia/content/CONTENT/<id>/` — script.md/prompt.md/metadata.json/audio.json, parsed by `platform/system/scripts/somia/content_spec.py`'s `load_content_spec()`. Run it against every content dir after a batch add; a malformed heading (e.g. a missing `## Text` section) fails silently unless checked.
...
## Operating notes
- After any bulk data change (a new batch of POIs, a new batch of somia CONTENT dirs), actually run the schema check — don't assume it's fine because the individual edits looked right. For kabukicho: `python -m pytest businesses/kabukicho_survival_map/app/tests -q`. For somia: run `load_content_spec()` over every `CONTENT/*` dir and report any that raise `ContentSpecError` or return an empty required field.
