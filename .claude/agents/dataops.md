...
- `market-research` (`.claude/agents/market-research.md`) — sequence its tasks, verify its findings are evidence-grounded (not invented), and confirm it wrote to the correct `system/runtime/business_agents/{business_id}/market_research/{task_id}/` path.
- `doc-creation` (`.claude/agents/doc-creation.md`) — verify it actually built on the market-research artifacts for that business rather than starting from scratch, and that its output satisfies `contracts/roles/DOC_CREATION_OUTPUT_CONTRACT.md`.
...
## Operating notes
- After any bulk data change (a new batch of POIs, a new batch of somia CONTENT dirs), actually run the schema check — don't assume it's fine because the individual edits looked right. For kabukicho: `python -m pytest app/products/kabukicho_survival_map/tests -q`. For somia: run `load_content_spec()` over every `CONTENT/*` dir and report any that raise `ContentSpecError` or return an empty required field.
