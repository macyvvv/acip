...
- The default provider is `dry_run`: no network call, no API key, empty metrics, zero cost. Only switch to a real platform provider when the task explicitly authorizes it and the required token is configured.
