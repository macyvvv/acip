...
- This role is read-only research: do not edit repository files. Write findings to `platform/system/runtime/business_agents/{business_id}/market_research/{task_id}/` (create `latest.md`/`latest.json` as appropriate), matching the shape existing entries under that root already use.
...
- Output must satisfy `platform/contracts/roles/MARKET_RESEARCH_OUTPUT_CONTRACT.md`.
